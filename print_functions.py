...
def colorize_print_value(value):
	if value < 20:
		return color.NEGATIVE + str('%-8.2s' % (value)) + color.END
	return color.POSITIVE + str('%-8.2s' % (value)) + color.END
...
def colorize_html_value(value,currency,colorize):
	...
	if value < 0 and colorize:
		...
		ret = ret + "width=\"70\"><font color=\"#FF0000\">" + str('%-8.2f' % (value))
	if value >= 0 and colorize:
		#ret = ret + "bgcolor=\"#00FF00\" width=\"70\"><font color=\"black\">" + str('%-8.2f' % (value))
		ret = ret + "width=\"70\"><font color=\"#00FF00\">" + str('%-8.2f' % (value))
	if not colorize:
...
def print_html_globals(d):
	print("<table cellspacing=\"3\" width=\"100%\"><tr><th width=\"150\">NAME</th><th width=\"70\">PRICE</th><th width=\"70\">%DAY</th></tr>")
	for v in d["globals"]:
